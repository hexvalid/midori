...
public class Log {
    final private static int MAX_LOGS = 100;
    ...
    public static void Print(t type, String s) {
        Platform.runLater(() -> {
            ...
            tf.setPrefSize(446, Region.USE_COMPUTED_SIZE);
            if (Main.Controller._log.getItems().size() > MAX_LOGS) {
                Main.Controller._log.getItems().remove(0);
            }
            Main.Controller._log.getItems().add(tf);
            Main.Controller._log.scrollTo(Main.Controller._log.getItems().size() - 1);
        });
    }

    static void TestLog() {
        Print(t.SCS, "Task is successfuly completed!");
        Print(t.DBG, "Debug info here: 001001011101011011");
        Print(t.INF, "This is a info message");
        Print(t.WRN, "Warning! Samething is a wrong! (Maybe not?)");

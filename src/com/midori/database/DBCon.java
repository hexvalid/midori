...
public class DBCon {
    ...
    final static String sqlInsertAccount = "insert into accounts(id, email, password, btcAddress, balance, rewardPoints, referrer, lastFPDate, fingerprint, fingerprint2, timeOffset, loginDate, signUpDate, signUpIP, fpPlayed, fpBonusReqCompleted, fpRpCost, emailConfirmed, stats, disableLottery, tfaEnabled, tfaSecret, useRPForFP, headerUserAgent, headerAcceptLanguage, sleepTimes, fpLatencyTimes, boosts, proxy, cookieStore, logs) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    final static String sqlGetAllAccounts = "select * from accounts";
    final static String sqlUpdateAccountForRoll = "update accounts set balance = ?, rewardPoints = ?, lastFPDate = ?, fpPlayed = ?, fpBonusReqCompleted = ?, fpRpCost  = ?, emailConfirmed = ?, stats = ?, disableLottery = ?, tfaEnabled = ?, cookieStore = ? where id = ?;";

...
public class DBCon {
    final static String sqlInitAccounts = "create table accounts(id integer not null constraint accounts_pk primary key, email text, password text, btcAddress text, balance double, rewardPoints integer, referrer integer, lastFPDate date, fingerprint text, fingerprint2 int, timeOffset int, loginDate date, signUpDate date, signUpIP text, fpPlayed int, fpBonusReqCompleted double, fpRpCost integer, emailStatus text, stats text, disableLottery boolean, tfaEnabled boolean, tfaSecret text, useRPForFP boolean, headerUserAgent text, headerAcceptLanguage text, sleepTimes text, fpLatencyTimes text, boosts text, proxy text, cookieStore blob, logs text);";
    ...
    final static String sqlInsertAccount = "insert into accounts(id, email, password, btcAddress, balance, rewardPoints, referrer, lastFPDate, fingerprint, fingerprint2, timeOffset, loginDate, signUpDate, signUpIP, fpPlayed, fpBonusReqCompleted, fpRpCost, emailStatus, stats, disableLottery, tfaEnabled, tfaSecret, useRPForFP, headerUserAgent, headerAcceptLanguage, sleepTimes, fpLatencyTimes, boosts, proxy, cookieStore, logs) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    final static String sqlGetAllAccounts = "select * from accounts";
    final static String sqlUpdateAccountForRoll = "update accounts set balance = ?, rewardPoints = ?, lastFPDate = ?, fpPlayed = ?, fpBonusReqCompleted = ?, fpRpCost  = ?, stats = ?, disableLottery = ?, cookieStore = ? where id = ?;";
